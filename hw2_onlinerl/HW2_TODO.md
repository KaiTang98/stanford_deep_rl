...
- [x] 本机继续用 HW1 的 `cs224r` 环境（已补 `pytest` / `hydra-core` / `omegaconf` / `pyyaml`；numpy + torch + MPS 可用）
- [x] 本机 `cs224r` 已安装 `wandb` 0.29；**登录还差一步**：在自己的终端跑 `wandb login`（见下）
- [ ] 本机 `wandb login`（用 GitHub 账号在网页登录后，把 authorize 页的 API key 贴进终端；**不要把 key 发到聊天里**）
...
- [x] `choose_action` — ε-greedy：以 ε 均匀随机，否则 greedy；**平局任意打破**
- [x] `greedy_action` — 代码里也是 `YOUR CODE HERE`（PDF 没单独列，但 `rollout_policy` 会调用；实现 `argmax_a Q(s,a)`）
- [x] `train_q_learning` 内层循环，每步:
  1. ε-greedy 选动作
...
1. **所有 N 个** critic 都对同一个 `y` 做 MSE，再求和/平均后反传
2. `utils.soft_update_params(self.critic, self.critic_target, self.critic_target_tau)`
...
1. 梯度只走 actor，**不要**更新 critic（对 Q 的调用保持 `torch.no_grad()` 或 detach）
